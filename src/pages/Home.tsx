...
const featureCards = [
  ...
  { icon: Image, title: "Photo Gallery", description: "Browse through cherished memories, special moments, and milestone celebrations.", link: "/gallery" },
  { icon: Calendar, title: "Upcoming Events", description: "Stay updated on family gatherings, celebrations, and important dates.", link: "/events" },
  { icon: UtensilsCrossed, title: "Family Recipes", description: "Explore treasured recipes and culinary traditions passed down through generations.", link: "/recipes" },
  { icon: BookOpen, title: "Family Blog", description: "Read updates, stories, and announcements from family members.", link: "/blog" },
  { icon: Mail, title: "Connect With Us", description: "Get in touch, share your thoughts, or leave a message in our guestbook.", link: "/contact" },
];
...
const Home = () => {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      const { data } = await supabase.from("site_content").select("*").in("section_key", ["home_intro", "newsletter"]);
      if (data) {
        const intro = data.find(d => d.section_key === "home_intro");
        const news = data.find(d => d.section_key === "newsletter");
        if (intro) setIntroContent({ title: intro.title || introContent.title, content: intro.content || introContent.content });
        if (news) setNewsletterContent({ title: news.title || newsletterContent.title, content: news.content || newsletterContent.content });
      }
    };
    fetchContent();
  }, []);
